  ...
  sendMessage(content: string, images?: string[]): void {
    ...
    this.api.sendChat(content).subscribe({
      ...
      error: (err) => {
        ...
        let errText = "Une erreur est survenue lors de la communication avec le serveur.";
        if (status === 404) {
          errText = "Aucun document pertinent trouvé pour cette question.";
        } else if (status === 0) {
          errText = "Impossible de joindre le serveur. Vérifiez que le backend est démarré sur `http://localhost:8000`.";
        }
